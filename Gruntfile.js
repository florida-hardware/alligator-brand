module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        meta: {
            site_name: 'Florida Hardware Farm Equipment',
    ...
    const Tasks = {
        ...
        templates: ['buildTemplates','buildGenerators'],
        jsbuild: ['concat:site', 'concat:utils', 'copy:js'],
        deploy: (target != 'dev') ? ['default','deploy'] : ['default'],
    ...
    grunt.registerTask("stage", "Deploy to staging/sync directory.", function() {
        grunt.file.copy("www", grunt.config.get('env.staging'));
    });
    */

    grunt.registerTask("deploy", "Deploy to GitHub.", function() {
        let done = this.async();
