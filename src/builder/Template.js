...
const RX_IFSIMPLE = /\{%IF([0-9]?)\s+([A-Za-z0-9_\.]*)\s*\}(.*?)\{%ENDIF(\1)\}/sg;
const RX_IFCOMP = /\{%IF([0-9]?)\s+([A-Za-z0-9_\.]*)\s*(==|>=|<=|!=)\s*([A-Za-z0-9_\."]*)\s*}(.*?)\{%ENDIF(\1)\}/sg;
...
function processSegment(tmpl, scope, preserve = false) {
	...
	// conditional
	output = output.replace(RX_IFSIMPLE, (w,n,g,t) => {
	...
	output = output.replace(RX_IFCOMP, (w,n,g,c,v,t) => {
